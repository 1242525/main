        ...
        <main className="flex-1 p-10 max-w-4xl">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <label
                htmlFor="fileUpload"
                className={`block border border-dashed border-white/10 rounded-lg p-6 text-center transition ${
                  isLoggedIn ? "hover:border-cyan-400 cursor-pointer" : "opacity-50 cursor-not-allowed"
                }`}
              >
                <div className="text-gray-500 mb-2">
                  {files.length > 0 ? "선택된 파일" : "파일을 드래그하거나 클릭하여 업로드"}
                </div>
                {files.length > 0 && (
                  <div className="mt-3 space-y-1 text-sm">
                    {files.map((file, i) => (
                      <div key={i} className="text-cyan-300">✔ {file.name}</div>
                    ))}
                  </div>
                )}
              </label>
